...
function NewClassModal({ entrenadores, catalogItems, collectionRates, defaultDate, onClose }) {
    ...
    function pickCatalogItem(e) {
        ...
        setItemDraft(d => ({ ...d, catalog_item_id: id, nombre: found?.nombre ?? '', precio: found ? String(found.precio) : '', usar_paquete: true }));
    }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <form onSubmit={submit} className="bg-white border border-zinc-200 rounded-xl shadow-lg p-5 w-full max-w-lg space-y-3 max-h-[92vh] overflow-y-auto">
                <h3 className="font-semibold text-zinc-800 text-sm">Nueva clase de entrenamiento</h3>
                ...
                <div className="flex gap-2 pt-1">
                    ...
                    <button type="submit" disabled={form.processing} className="flex-1 bg-zinc-900 text-white py-1.5 rounded-lg text-sm font-medium hover:bg-zinc-700 disabled:opacity-50 transition-colors">
                        {form.processing ? 'Agendando...' : 'Agendar clase'}
                    </button>
                </div>
            </form>
        </div>
...
function ClassCard({ appt }) {
    const bg = estadoBg[appt.estado] ?? 'bg-zinc-400';
    return (
        <Link href={route('training.show', appt.id)}>
            <div className={`rounded-lg p-2.5 text-white text-xs cursor-pointer hover:opacity-90 transition-opacity ${bg}`}>
                ...
                {appt.entrenador && <div className="opacity-75 mt-0.5 truncate">{appt.entrenador}</div>}
            </div>
        </Link>
    );
}

function MobileClassItem({ appt }) {
                ...
                <div className="flex-1 flex items-center gap-3 px-4 py-3">
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-zinc-400 mt-0.5 truncate">
                            {[appt.entrenador].filter(Boolean).join(' · ')}
                        </div>
                    </div>
...
export default function TrainingIndex({ appointments, weekStart, entrenadores, catalogItems, collectionRates = [] }) {
    ...
    function goWeek(delta) {
        router.get(route('training.index'), { week_start: addDays(weekStart, delta * 7) }, { preserveState: false, replace: true });
    }
    ...
    return (
        <TenantLayout title="Entrenamientos">
            ...
            <div className="flex justify-between items-center mb-5">
                <h2 className="text-xl font-semibold text-zinc-900 tracking-tight">Entrenamientos</h2>
            ...
            <div className="flex items-center justify-center gap-4 mb-6">
                ...
                <div className="text-center min-w-[160px]">
                    ...
                    {isCurrentWeek
                        ...
                        : <button onClick={() => router.get(route('training.index'), {}, { preserveState: false, replace: true })} className="text-xs text-zinc-500 hover:text-zinc-700 mt-0.5 transition-colors">Ir a esta semana</button>
                    }
                </div>
                <button onClick={() => goWeek(1)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-zinc-100 text-zinc-500 text-xl font-light transition-colors">›</button>
            </div>
                ...
                <div className="grid grid-cols-7 gap-2">
                    {weekDays.map(day => (
                        <div key={day} className="space-y-1.5 min-h-[3rem]">
                            {byDay[day].map(a => <ClassCard key={a.id} appt={a} />)}
            ...
            <div className="sm:hidden">
                ...
                <div className="space-y-3">
                    {byDay[activeMobileDay].map(a => <MobileClassItem key={a.id} appt={a} />)}
                    {byDay[activeMobileDay].length === 0 && (
                        <div className="bg-white border border-zinc-100 shadow-sm rounded-xl p-8 text-center text-zinc-400 text-sm">Sin clases este día</div>
                    )}

...
function RecoleccionConfigTab({ recoleccionConfig }) {
    const form = useForm({
        no_show_restaura_credito: recoleccionConfig?.no_show_restaura_credito ?? true,
    });

    function handleSubmit(e) {
        e.preventDefault();
        form.post(route('settings.recoleccion.update'));
    }

    return (
        <div className="max-w-md bg-white border border-zinc-100 shadow-sm rounded-xl p-5 space-y-5">
            <div>
                <h3 className="font-semibold text-zinc-700">Recolección</h3>
                <p className="text-xs text-zinc-400 mt-0.5">Define qué pasa cuando el chofer marca una parada como "no se presentó" (la mascota o el dueño no estaban).</p>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
                <label className="flex items-center justify-between cursor-pointer">
                    <span className="text-sm font-medium text-zinc-700">
                        Regresar el crédito en un no-show
                        <span className="block text-xs font-normal text-zinc-400 mt-0.5">
                            Si está activo, se libera el crédito de membresía o paquete usado (igual que al cancelar). Si lo desactivas, el crédito se queda consumido.
                        </span>
                    </span>
                    <button type="button" role="switch" aria-checked={form.data.no_show_restaura_credito}
                        onClick={() => form.setData('no_show_restaura_credito', !form.data.no_show_restaura_credito)}
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors shrink-0 ml-3 ${form.data.no_show_restaura_credito ? 'bg-zinc-900' : 'bg-zinc-200'}`}>
                        <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${form.data.no_show_restaura_credito ? 'translate-x-6' : 'translate-x-1'}`} />
                    </button>
                </label>
...
export default function SettingsIndex({ categories, items, paymentMethods, stations, checklistItems, ticketConfig, generalConfig, walkConfig, recordatoriosConfig, recoleccionConfig, responsivaConfig, responsivaConfigs, planInfo, mercadoPagoConfig, teamMembers, razas, sucursales }) {
    ...
    return (
        <TenantLayout title="Configuración">
            ...
            {tab === 'recoleccion' && <RecoleccionConfigTab recoleccionConfig={recoleccionConfig ?? { no_show_restaura_credito: true }} />}

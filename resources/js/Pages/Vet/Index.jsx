...
function NewConsultaModal({ veterinarios, catalogItems, collectionRates, defaultDate, onClose }) {
    ...
    function selectPet(pet) { setSelectedPet(pet); setPetResults([]); form.setData('pet_id', pet.id); }
    ...
    function pickCatalogItem(e) {
        ...
        setItemDraft(d => ({ ...d, catalog_item_id: id, nombre: found?.nombre ?? '', precio: found ? String(found.precio) : '' }));
    }
    function addItem() {
        if (!itemDraft.catalog_item_id) return;
    ...
    function submit(e) { e.preventDefault(); form.post(route('vet.store'), { onSuccess: onClose }); }
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <form onSubmit={submit} className="bg-white border border-zinc-200 rounded-xl shadow-lg p-5 w-full max-w-lg space-y-3 max-h-[92vh] overflow-y-auto">
                <h3 className="font-semibold text-zinc-800 text-sm">Nueva consulta veterinaria</h3>
                ...
                <div>
                    ...
                    <div className="grid grid-cols-12 gap-1">
                        ...
                        <div className="col-span-5 flex items-center px-2 text-xs text-zinc-700 bg-zinc-50 border border-zinc-200 rounded-lg truncate">
                            {itemDraft.catalog_item_id ? `${itemDraft.nombre} — $${Number(itemDraft.precio || 0).toFixed(2)}` : ''}
                        </div>
                ...
                <div>
                    ...
                    <textarea className="w-full border-gray-300 rounded-lg text-sm resize-none" rows={2}
                        value={form.data.notas_internas} onChange={e => form.setData('notas_internas', e.target.value)} />
                </div>
                ...
                <div className="flex gap-2 pt-1">
                    ...
                    <button type="submit" disabled={form.processing} className="flex-1 bg-zinc-900 text-white py-1.5 rounded-lg text-sm font-medium hover:bg-zinc-700 disabled:opacity-50 transition-colors">
                        {form.processing ? 'Agendando...' : 'Agendar consulta'}
                    </button>
                </div>
            </form>
        </div>
    ...
    return (
        <Link href={route('vet.show', appt.id)}>
            <div className={`rounded-lg p-2.5 text-white text-xs cursor-pointer hover:opacity-90 transition-opacity ${bg}`}>
                ...
                {appt.veterinario && <div className="opacity-75 mt-0.5 truncate">{appt.veterinario}</div>}
            </div>
        </Link>
    );
}
...
export default function VetIndex({ appointments, weekStart, veterinarios, catalogItems, pendingRequests = [], collectionRates = [] }) {
    ...
    function goWeek(delta) {
        router.get(route('vet.index'), { week_start: addDays(weekStart, delta * 7) }, { preserveState: false, replace: true });
    }
        ...
        <TenantLayout title="Veterinaria">
            ...
            <div className="flex items-center justify-center gap-4 mb-6">
                ...
                <div className="text-center min-w-[160px]">
                    ...
                    {isCurrentWeek
                        ...
                        : <button onClick={() => router.get(route('vet.index'), {}, { preserveState: false, replace: true })} className="text-xs text-zinc-500 hover:text-zinc-700 mt-0.5 transition-colors">Ir a esta semana</button>
                    }
                </div>
                <button onClick={() => goWeek(1)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-zinc-100 text-zinc-500 text-xl font-light transition-colors">›</button>
            </div>
                ...
                <div className="grid grid-cols-7 gap-2">
                    {weekDays.map(day => (
                        <div key={day} className="space-y-1.5 min-h-[3rem]">
                            {byDay[day].map(a => <ConsultaCard key={a.id} appt={a} />)}
            ...
            <div className="sm:hidden">
                ...
                <div className="space-y-3">
                    {byDay[activeMobileDay].map(a => <MobileConsultaItem key={a.id} appt={a} />)}
                    {byDay[activeMobileDay].length === 0 && (
                        <div className="bg-white border border-zinc-100 shadow-sm rounded-xl p-8 text-center text-zinc-400 text-sm">Sin consultas este día</div>
                    )}

...
class AppointmentController extends Controller
{
    ...
    public function complete(Request $request, Appointment $appointment): RedirectResponse
    {
        ...
        DB::transaction(function () use ($appointment, $data, &$ticket) {
            ...
            // así que no se facturan de nuevo. El cargo de recolección, si se
            // solicitó, ya está entre los items (se agregó al agendar).
            ...
            // Marca la recolección vinculada como facturada en este ticket (si
            // aplica) para trazabilidad, aunque su cargo ya viaja como item.
            $recoleccion = $this->collectionBookings->findForOrigin('appointment', $appointment->id);
    ...
    /**
     * Cancela la recolección vinculada a esta cita (si sigue pendiente) al cancelarse o no
     * presentarse. No restaura el crédito de paquete aquí: la tarifa de la recolección
     * también viaja como AppointmentItem, así que restorePackageCredits() ya lo restaura
     * al recorrer los items de la cita — restaurarlo también aquí lo duplicaría.
     */

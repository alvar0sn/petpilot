...
class VetController extends Controller
{
    ...
    public function complete(Appointment $appointment): RedirectResponse
    {
        ...
        DB::transaction(function () use ($appointment, $rec, &$ticket) {
            ...
            // Los items propios de veterinaria nunca se cubren con paquete (no hay
            // mecanismo para eso aquí) — solo el cargo de recolección, si viene
            // mirroreado como item con package_credit_id, puede estar cubierto.
    ...
    /**
     * Cancela la recolección vinculada a esta consulta (si sigue pendiente). A diferencia
     * de grooming/entrenamiento, veterinaria no tiene un flujo que restaure créditos de
     * paquete de sus items al cancelar, así que aquí sí se restaura directamente.
     */
    private function cancelLinkedCollection(Appointment $appointment, string $motivo): void
    {
        $this->collectionBookings->cancelForOrigin('appointment', $appointment->id, $motivo, true);
    }
